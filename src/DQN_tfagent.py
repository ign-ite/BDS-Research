...
# Data Collection
def collect_step(environment, policy, buffer, cpu_utilization_list, mem_utilization_list, adherence_list, policy_state=None):
...
def collect_data(env, policy, buffer, steps, cpu_utilization_list, mem_utilization_list, adherence_list):
    ...
    for _ in range(steps):
        policy_state = collect_step(env, policy, buffer, cpu_utilization_list, mem_utilization_list, adherence_list, policy_state)
...
def train_dqn(
    ...
    # ***Agent***
    if constants.use_recurrent_policy:
    ...
    dataset = replay_buffer.as_dataset(
        ...
        num_steps=20 if constants.use_recurrent_policy else 2).prefetch(3)
    ...
    for _ in range(num_iterations):
        collect_policy_state = agent.collect_policy.get_initial_state(train_env.batch_size)

        # Collect a few steps using collect_policy and save to the replay buffer.
        for _ in range(collect_steps_per_iteration):
            collect_policy_state = collect_step(
                train_env,
